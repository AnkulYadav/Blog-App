...
const handleOpenSubMenu = (e) => {
    ...
    e.target.parentElement.nextSibling.classList.toggle('block')
    e.target.parentElement.nextSibling.classList.add('relative')

    console.log(e.target.parentElement.nextSibling.nextChildren)
}
        ...
        <li className='relative group'>
            {
                item.type === 'link' ?
                    ...
                    <>
                        ...
                        {item.name}
                        <MdKeyboardArrowDown className='inline-block ml-1' onClick={handleOpenSubMenu} />
                        </a>
                        <div className='hidden transition-all duration-500 pt-4 absolute bottom-0 right-0 transform translate-y-full group-hover:block w-max'>
                            <ul className='flex flex-col shadow-lg rounded-lg overflow-hidden'>
                                {item.dropdownItems.map((page, index) => <li key={index} className='bg-white hover:bg-blue-500 hover:text-white transition-all duration-300'><a href='/' className='px-4 py-2 block'>{page.name}</a></li>)}
                            </ul>
                        </div>
                    </>}
        </li>
